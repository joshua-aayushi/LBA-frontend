...
export const tripsTableColumns: ColumnDef<Trip>[] = [
  ...
  //   header: ({ column }) => (
  //     <DataTableColumnHeader column={column} title="Driver Phone" />
  //   ),
  //   cell: ({ row }) => {
  //     const phone = row.original.driverDetails.phone
  //     return (
  //       <div className="flex w-[144px] space-x-2">
  ...
  },
  {
    accessorKey: "expense",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Expense Amount" />
    ),
    cell: ({ row }) => {
      const expense = formatNumberToIndianReadble(row.original.expenseDetails?.totalAmount ?? 0)
      return (
        <div className="flex w-[144px] space-x-2">
          <span className="truncate font-medium">₹ {expense}</span>
        </div>
      );
    },
  },
  {
    accessorKey: "primaryDetails.lrNumber",
    header: ({ column }) => (
    ...
    cell: ({ row }) => {
      return (
        <div className="flex w-[144px] space-x-2">
          <span className="truncate font-medium">{row.getValue("primaryDetails_lrNumber")}</span>
        </div>
      );
    },
  },
  {
    accessorKey: "primaryDetails.status",
    header: ({ column }) => (
