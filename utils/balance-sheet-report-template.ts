...
export function balanceSheetReportTemplate(
  ...
  return `<!DOCTYPE html> <html lang="en"> <head> <meta charset="UTF-8"> <meta name="viewport" content="width=device-width,initial-scale=1"> <title>Bank Statement Report</title> <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KK94CHFLLe+nY2dmCWGMq91rCGa5gtU4mk92HdvYe+M/SXH301p5ILy+dN9+nJOZ" crossorigin="anonymous"> <link rel="preconnect" href="https://fonts.googleapis.com"> <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin> <link href="https://fonts.googleapis.com/css2?family=Inter:wght@100..900&display=swap" rel="stylesheet"> <style> :root { --body-font: "Inter", sans-serif; } body { --bs-font-sans-serif: "Inter", sans-serif; --bs-body-font-family: var(--bs-font-sans-serif); --bs-body-font-size: 0.6rem; --bs-body-font-weight: 400; --bs-body-line-height: 2; --bs-body-color: #41403e; --bs-primary: #0070e4; --bs-primary-rgb: 0, 112, 228; --bs-border-color: #eeeeee; } </style> </head> <body> <section id="bank-statement"> <div class="m-5"> <div class="text-center"> <img src="${
    user.companyDetails.companyLogo
  }" alt="${
    user.companyDetails.name
  }"> </div> <div class="text-center border-top border-bottom my-5 py-3"> <h2 class="display-5 fw-bold">Bank Statement</h2> <div class="d-flex justify-content-between"> <p class="m-0">From Date: ${reverseDateString(
    from
  )}</p> <p class="m-0">To Date: ${reverseDateString(
    to
  )}</p> </div> </div> <table class="table border my-5"> <thead> <tr class="bg-primary-subtle"> <th scope="col">Sr. No.</th> <th scope="col">Date</th> <th scope="col">Title</th> <th scope="col">A/C Name</th> <th scope="col">Credit (₹)</th> <th scope="col">Debit (₹)</th> <th scope="col">Status</th> </tr> </thead> <tbody> ${items
    .map(
      (item, index) =>
        ` <tr> <th scope="row">${index + 1}</th> <td>${reverseDateString(
          item.date
        )}</td> <td>${item.title}</td> <td>${item.accountName}</td> <td>${
          item.status === "CREDIT"
            ? `₹ ${formatNumberToIndianReadble(item.amount)}`
            : "-"
        }</td> <td>${
          item.status === "DEBIT"
            ? `₹ ${formatNumberToIndianReadble(item.amount)}`
    ...
    .join(
      "\n"
    )} <tr> <td colspan="4" class="fw-bold text-end">Total Credited Amount</td> <td colspan="2" class="text-end">₹ ${formatNumberToIndianReadble(
    totalCreditedAmount
  )}</td> </tr> <tr> <td colspan="4" class="fw-bold text-end">Total Debited Amount</td> <td colspan="2" class="text-end">₹ ${formatNumberToIndianReadble(
    totalDebitedAmount
  )}</td> </tr> <tr> <td colspan="4" class="fw-bold text-end">Balance Amount</td> <td colspan="2" class="text-end">₹ ${formatNumberToIndianReadble(
    balanceAmount
